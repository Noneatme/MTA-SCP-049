...
function Trigger:CheckForMusic(index)
	...
	elseif(index == 14) then
		...
		effects:AddRedEffect(10*60*60*1000)
		alien:Appear(336.82791137695, 181.82263183594, 1014.1875, 0, false, true);
...
function Trigger:PlayMusic(level)
	...
	if(level == 2) then
		self.music = soundManager:PlaySound("http://noneatme.de/sounds/mus_trt.mp3", false, "music");
...
function Trigger:PlayDoorTrigger(index)
	if not(self.doorTriggerPlayed[index]) then
		if(index == 3) then
			...
			self.music = soundManager:PlaySound("http://noneatme.de/sounds/music2.ogg", true, "music");
...
function Trigger:Constructor(...)
	...
	self.chasePlayerWin = function(so)
		if(getElementData(doors.doors[14], "locked") == false) then
			...
			alien:Appear(366.99230957031, 178.576171875, 1008.3828125, 201, false, false);
